test_that("'competition_dist' must be specified in compile_model() if missing", {
  ...
  expect_error(compile_model(populations = list(p), generation_time = 30, resolution = 11e3, path = tempfile(), overwrite = TRUE, direction = "backward"),
               "Parameter 'competition_dist' missing", fixed = TRUE)
  expect_silent(compile_model(competition_dist = 50e3, populations = list(p), generation_time = 30, resolution = 10e3, path = tempfile(), overwrite = TRUE, direction = "backward"))
})
...
test_that("'mate_dist' must be specified in compile_model() if missing", {
  ...
  expect_error(compile_model(populations = list(p), generation_time = 30, resolution = 10e3, path = tempfile(), overwrite = TRUE, direction = "backward"),
               "Parameter 'mate_dist' missing", fixed = TRUE)
  expect_silent(compile_model(mate_dist = 50e3, populations = list(p), generation_time = 30, resolution = 10e3, path = tempfile(), overwrite = TRUE, direction = "backward"))
})
...
test_that("'dispersal_dist' must be specified in compile_model() if missing", {
  ...
  expect_silent(compile_model(dispersal_dist = 50e3, populations = list(p), generation_time = 30, resolution = 10e3, path = tempfile(), overwrite = TRUE, direction = "backward"))
})
...
test_that("'competition_dist', 'mate_dist', and 'dispersal_dist' do not have to be specified in compile_model() if already present", {
  ...
  expect_silent(compile_model(populations = list(p), generation_time = 30, resolution = 10e3, path = tempfile(), overwrite = TRUE, direction = "backward"))
})
...
test_that("invalid blank maps are prevented", {
  ...
  expect_error(compile_model(pop, tempdir(), generation_time = 1, competition_dist = 1,
...
test_that("`dir is deprecated in favor of `path`", {
  ...
  expect_warning(model <- compile_model(p, dir = file.path(tempdir(), "dir-rerouted-to-path"), generation_time = 30, overwrite = TRUE),
