...
slim(model, sequence_length = 1000000, recombination_rate = 0, random_seed = 42) %>% ts_write(ts_file)

ts <- ts_read(ts_file, model) %>% ts_recapitate(Ne = 100, recombination_rate = 0, random_seed = 42) %>%
  ts_simplify(simplify_to = paste0("POP_", seq(1, 10)))
