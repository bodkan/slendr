...
test_that("number of iterations must be a non-negative, non-zero integer", {
  ...
  ts <- msprime(model, schedule = schedule, sequence_length = 10e6, recombination_rate = 1e-8)
...
test_that("ts_extend() gives the same tables as the Python method extend_haplotypes()", {
  ts <- msprime(model, schedule = schedule, sequence_length = 10e6, recombination_rate = 1e-8, coalescent_only = FALSE)
