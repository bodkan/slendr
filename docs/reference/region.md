...
Creates a geographic region (a polygon) on a given map and gives it a
name. This can be used to define objects which can be reused in multiple
places in a slendr script (such as `region` arguments of `population`)
without having to repeatedly define polygon coordinates.
