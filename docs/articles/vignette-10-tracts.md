...
function is extremely experimental, only minimally tested, and its
functionality is expected to change quite a bit. Please wait for the
release of the next major version of *slendr* (which should include a
more developed
[`ts_tracts()`](https://bodkan.net/slendr/reference/ts_tracts.md))
before you use this in your own work.**
...
[`ts_tracts()`](https://bodkan.net/slendr/reference/ts_tracts.md) and
this vignette describes its use on a simple toy model of Neanderthal and
...
[`ts_tracts()`](https://bodkan.net/slendr/reference/ts_tracts.md) which
serves as a simplified R-friendly interface to the Python method
...
use [`ts_tracts()`](https://bodkan.net/slendr/reference/ts_tracts.md)
without running a *slendr* simulation first):
