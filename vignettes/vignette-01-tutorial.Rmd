...
Since we instructed *slendr* to track ancestry proportions above, we can visualize them after the simulation is over using the built-in `plot_ancestry()` function. This functionality is quite bare-bones for now and the way diagnostics are generated is probably subject to change.

```{r plot_ancestries}
plot_ancestry(model = model)
```
