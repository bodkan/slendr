...
```{r eval = FALSE}
geneflows <- gene_flow(from = eur, to = afr, rate = 0.1, start = 20000, end = 15000)
```
...
model <- compile_model(
  ...
  path = paste0(tempfile(), "_tutorial-model")
)
```
...
```{r}
loaded_model <- read_model(file.path(tempdir(), "tutorial-model"))
```
...
```{r, eval = FALSE}
animate_model(model = model, steps = 70, width = 500, height = 300)
```

If an argument `gif = ...` is provided, `animate_model()` will save the animation to the specified location as a GIF file. In this case, the command would produce the animation you can see on the *slendr* package [landing page](https://www.slendr.net).
