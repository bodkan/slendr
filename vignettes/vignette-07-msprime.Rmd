...
slim(
  ...
  method = "batch", verbose = TRUE, seed = RANDOM_SEED
)
```
...
inds <- ts_samples(ts) %>% filter(pop == "EUR")
...
r.inds = r.inds.assign(ancestry_tskit_msprime = 1 - msp_f4_num / msp_f4_den)
```
