...
extension <- substitute(extension_path, s = 0.01, onset_time = 250)
model <- compile_model(pop, generation_time = 1, simulation_length = 1000, extension = extension)

arguments <- list(output_file = "~/Desktop/output.tsv", target_pop = "pop")

slim(model, sequence_length = 1e6, recombination_rate = 1e-8, arguments = arguments)
...
extension <- substitute(extension_path, s = 0.1, onset_time = 15000)
model <- compile_model(
  populations = list(afr, ooa, ehg, eur, ana, yam),
  gene_flow = gf, generation_time = 30,
  extension = extension 
...
arguments <- list(output_file = "~/Desktop/output_eur.tsv", target_pop = "EUR")
ts <- slim(model, sequence_length = 1e6, recombination_rate = 1e-8, arguments = arguments)

arguments <- list(output_file = "~/Desktop/output_ana.tsv", target_pop = "ANA")
ts <- slim(model, sequence_length = 1e6, recombination_rate = 1e-8, arguments = arguments)

trajectory <- read.table("~/Desktop/output_eur.tsv", header = TRUE)
plot(-trajectory$time, trajectory$frequency, type = "l", xlim = c(-15e3, 0))

trajectory <- read.table("~/Desktop/output_ana.tsv", header = TRUE)
plot(-trajectory$time, trajectory$frequency, type = "l", xlim = c(-15e3, 0))
```
