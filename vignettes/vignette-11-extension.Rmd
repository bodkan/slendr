...
slim(model_eur, sequence_length = 1e6, recombination_rate = 1e-8, output_dir = "~/Desktop", verbose = TRUE)
```
...
```{r}
slim(model, recombination_rate = 1e-8, samples = samples, output_dir = "~/Desktop/asdf")
```
...
freqs <- read_tsv("~/Desktop/output_frequencies.tsv") %>% mutate(pos = pos %% 5e6)

ggplot() +
  # geom_line(data = freqs, aes(pos, freq, group = gene), color = "gray", alpha = 0.75) +
  geom_line(data = group_by(freqs, pos) %>% summarise(freq = 100 * mean(freq)), aes(pos, freq)) +
...
<!-- ```{r} -->
<!-- ts <- ts_load(file = "~/Desktop/output.trees", model) %>% -->
<!--   ts_recapitate(Ne = 1000, recombination_rate = 1e-8) %>% -->
<!--   ts_mutate(mutation_rate = 1e-8, keep_existing = FALSE) %>% -->
<!--   ts_simplify() -->

<!-- ts_save(ts, "~/Desktop/output_mutated.trees") -->
<!-- ``` -->

<!-- ```{r} -->
<!-- ts <- ts_load("~/Desktop/output_mutated.trees", model) -->
<!-- ``` -->
...
windows <- seq(from = window_length, to = n_genes * gene_length - 1, by = window_length)
...
ggplot(div_df) +
  ...
  labs(x = "coordinate along a gene", y = "genetic divergence",
