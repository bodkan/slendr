...
ancestor <- population("ancestor", time = 650e3, N = 5000, remove = 599e3)
...
initialize() {
  initializeMutationType("m1", 0.5, "f", -0.001); // deleterious mutations
  ...
  substitute(total_length = 1e6, exon_length = 10e3, gap_length = 10e3)
```
...
```{r}
nea_samples <- schedule_sampling(model, times = 50000, list(nea, 10))
modern_samples <- schedule_sampling(model, times = 0, list(eur, 100), list(afr, 100))
...
```{r}
windows <- seq(from = 0, to = 1e6, by = 50000)
