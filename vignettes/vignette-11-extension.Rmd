...
```{r}
# ancestral population of Neanderthals and anatomically modern humans
ancestor <- population("ancestor", time = 650e3, N = 10000, remove = 599e3)
...
initialize() {
  ...
  initializeGenomicElementType("g2", m1, 1.0); // 'exon' genomic type
  ...
  // odd elements = starts, even elements == ends
  exon_starts = integerMod(seqAlong(exons), 2) == 0;
  exon_ends = integerMod(seqAlong(exons), 2) == 1;

  initializeGenomicElement(g2, exons[exon_starts], exons[exon_ends]);
  ...
  substitute(total_length = 10e6, exon_length = 100e3, gap_length = 100e3)
...
# slim(model, verbose = TRUE, method = "gui", output = FALSE)
```

```{r}
ancestor <- population("ancestor", time = 800e3, N = 10000, remove = 599e3)
...
plot_model(model_for_plot, proportions = TRUE, order = c("NEA", "ancestor", "EUR", "AFR"))
```

```{r}
ts <- slim(model, output = "purifying.trees", verbose = TRUE)
```
...
while (start < total_length) {
  ...
  breakpoints <- c(breakpoints, start, end)
...
f4 <- ts_f4(ts, W = samples["AFR"], X = samples["EUR"], Y = samples["NEA"], Z= samples["chimp"], windows = breakpoints)$f4[[1]]

exon_f4 <- f4[c(TRUE, FALSE)]
gap_f4 <- f4[c(TRUE, FALSE)]

df <- rbind(
  dplyr::tibble(f4 = exon_f4, region = "exon"),
  dplyr::tibble(f4 = gap_f4, region = "gap")
)
```

```{r}
group_by(df, region) %>% summarise(mean(f4))
```

```{r}
ggplot(df) + geom_jitter(aes(region, f4, color = region))
```
