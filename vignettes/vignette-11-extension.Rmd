...
Another very useful pair of _slendr_ constants are `SIMULATION_START` and `SIMULATION_END`, which specify at which time points (in ticks!) does a given _slendr_ model start and end. The latter can be partifularly helpful for customized output callbacks.
...
nea <- population("NEA", time = 600e3, N = 1, remove = 40e3)
...
extension <- r"(
initialize() {
  // model parameters to be substitute()'d from R below
  ...
  defineConstant("uniform_rate", {{uniform_rate}});
...
initialize() {
  ...
  for (i in seqLen(n_genes)) {
    ...
    rates = c(rates, {{uniform_rate}}, 0.5);
...
```{r}
extension_sdel <- extension %>%
  substitute(
    introgression_time = 55000, s = -0.001, uniform_rate = 1e-8,
    gene_length = 5e6, n_genes = 100, n_markers = 100,
    output_file = "~/Desktop/output_sdel.tsv"
  )
```
...
model <- compile_model(
  ...
  generation_time = 30,
  extension = extension_sdel
)

# plot_model(model, proportions = TRUE, order = c("NEA", "ancestor", "EUR", "AFR"), log = T)
plot_model(model, proportions = TRUE, order = c("NEA", "EUR"), log = T)
```
...
# modern_samples <- schedule_sampling(model, times = 0, list(afr, 100), list(eur, 100))
...
# ts <- slim(model, output = "purifying.trees", samples = samples, verbose = TRUE)
slim(model, output = FALSE, samples = samples, verbose = TRUE)

end <- Sys.time()
end - start # 18 minutes
```

<!-- ```{r} -->
<!-- ts <- ts_load("purifying.trees", model) %>% -->
<!--   ts_recapitate(Ne = 1000, recombination_rate = 1e-8) %>% -->
<!--   ts_mutate(mutation_rate = 1e-8, keep_existing = FALSE) %>%  -->
<!--   ts_simplify() -->
<!-- ``` -->

<!-- ```{r} -->
<!-- ts_save(ts, "purifying_mutated.trees") -->
<!-- ``` -->

<!-- ```{r} -->
<!-- ts <- ts_load("purifying_mutated.trees", model) -->
<!-- ``` -->
...
freqs <- read_tsv("~/Desktop/output_sdel.tsv") %>% mutate(pos = pos %% 5e6)

ggplot() +
  # geom_line(data = freqs, aes(pos, freq, group = gene), color = "gray", alpha = 0.75) +
  geom_line(data = group_by(freqs, pos) %>% summarise(freq = mean(freq)), aes(pos, freq)) +
  geom_vline(xintercept = 5e6 / 2, linetype = "dashed", color = "red") +
  expand_limits(y = 0)
```
...
The above implies that if we compute Neanderthal-Eurasian divergence (as a simplified introgression-detection statistic), we should observe lower introgression levels within "exons" than within "gaps", because purifying selection should be removing Neanderthal introgressed DNA from those regions under selective constraint compared to neutral regions. Let's verify this.
...
```{r}
tracts <- ts_tracts(ts, census = 55000, source = "NEA")

tracts_ <- tracts %>% 
  mutate(gene = round(((left + right) / 2) / gene_length))
         # left = left %% gene_length,
         # right = right %% gene_length)
```

```{r}
tracts_ %>%
ggplot +
  # geom_segment(aes(x = left, xend = right, y = gene, yend = gene + 1)) +
  geom_vline(aes(xintercept = left)) +
  coord_cartesian(xlim = c(0, total_length))
```
...
genes <- unlist(lapply(seq_len(total_length / gene_length),
                       function(window_i) rep(window_i, gene_length / window_length)))

windows <- seq(from = 0, to = total_length - 1, by = window_length)
div_afr <- ts_divergence(ts, samples[c("AFR", "NEA")], windows = windows)$divergence[[1]]
div_eur <- ts_divergence(ts, samples[c("EUR", "NEA")], windows = windows)$divergence[[1]]

div <- rbind(
  tibble(pop = "afr", gene = genes, div = div_afr),
  tibble(pop = "eur", gene = genes, div = div_eur)
) %>% group_by(pop, gene) %>% mutate(window = 1:n())
```

```{r}
ggplot(div, aes(window, div, color = gene, group = gene)) +
  geom_vline(xintercept = (gene_length / window_length) / 2,
...
ggplot(aes(window, div, color = pop)) +
  geom_vline(xintercept = (gene_length / window_length) / 2,
             linetype = "dashed", color = "red") +
  geom_line()
```
...
```{r}
# ancestor of Neanderthals and anatomically modern humans
ancestor <- population("ancestor", time = 650e3, N = 1000, remove = 599e3)
...
```{r}
extension <-
  system.file("extdata", "extension_introgression.txt", package = "slendr") %>%
  substitute(s = -0.001, introgression_time = 55000, n_markers = 100, output_file = "./output.tsv")
```
...
```{r}
slim(model, output = FALSE, samples = samples, method = "gui")
```
...
freqs <- read_tsv("~/Desktop/output.tsv")
