...
# chimp <- population("chimp", time = 6.1e6, N = 2)

# ancestor <- population("ancestor", parent = chimp, time = 6e6, N = 5, remove = 599e3)
ancestor <- population("ancestor", time = 650e3, N = 5, remove = 599e3)

# two populations of anatomically modern humans: Africans and Europeans
afr <- population("AFR", parent = ancestor, time = 600e3, N = 5)
...
tick(60000) early() {
  ...
  substitute(total_length = 1000e6, s = -0.005, gene_length = 1e6);
```
...
```{r}
ts <- slim(model, verbose = TRUE, method = "gui")
```

```{r}
start <- Sys.time()

ts <- slim(model, output = "purifying.trees", samples = samples, verbose = TRUE, method = "gui")
...
```{e}
ts <- ts_recapitate(ts, Ne = 5000, recombination_rate = 1e-8) %>%
...
windows <- seq(from = 0, to = 100e6, by = 100e3)
div_afr_nea <- ts_divergence(ts, samples[c("AFR", "NEA")], windows = windows)$divergence[[1]]
div_eur_nea <- ts_divergence(ts, samples[c("EUR", "NEA")], windows = windows)$divergence[[1]]

plot(div_afr_nea, type = "l", col = "darkgray")
lines(div_eur_nea, col = "red")

div <- rbind(
  tibble(pop = "AFR", region = "exon", div = div_afr_nea[c(TRUE, FALSE)]),
  tibble(pop = "AFR", region = "intron", div = div_afr_nea[c(FALSE, TRUE)]),
  tibble(pop = "EUR", region = "exon", div = div_eur_nea[c(TRUE, FALSE)]),
  tibble(pop = "EUR", region = "intron", div = div_eur_nea[c(FALSE, TRUE)])
)

ggplot(div) + geom_boxplot(aes(region, div)) + facet_wrap(~ pop)
```
