...
initialize() {
  initializeMutationType("m1", 0.5, "f", {{s}}); // deleterious mutation type
  initializeGenomicElementType("g1", m1, 1.0); // genomic type of 'genes'
  ...
  while (start < total_length) {
    ...
    // uniform recombination within the gene, followed by a 0.5 recombination breakpoint
  ...
  gene_starts_idx = integerMod(seqAlong(genes), 2) == 0;
  gene_ends_idx = integerMod(seqAlong(genes), 2) == 1;
  defineConstant("gene_starts", genes[gene_starts_idx]);
  defineConstant("gene_ends", genes[gene_ends_idx]);

  // set up all the exons at once
  initializeGenomicElement(g1, gene_starts, gene_ends);
...
tick(60000) early() {
  log_output("adding fixed deleterious mutations to Neanderthals");

  gene_centers = asInteger((gene_ends + gene_starts) / 2);
  population("NEA").genomes.addNewDrawnMutation(m1, position = gene_centers);
}
)" %>%
  # substitute values of the {{placeholder}} parameters above
  substitute(total_length = 2000e6, s = -0.0005, gene_length = 2e6)
```
