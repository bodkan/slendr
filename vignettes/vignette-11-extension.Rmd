...
afr <- population("AFR", parent = ancestor, time = 600e3, N = 100)
eur <- population("EUR", parent = afr, time = 80e3, N = 10000)
...
end - start # 41 minutes
```

```{e}
ts <- ts_recapitate(ts, Ne = 1000, recombination_rate = 1e-8) %>%
  ts_mutate(mutation_rate = 1e-8, keep_existing = FALSE)

ts_save(ts, "purifying_mutated.trees")
# plot(ts_table(ts, "sites")$position, rep(1, length(ts_table(ts, "sites")$position)), pch = ".")
```
...
genes <- unlist(lapply(seq_len(total_length / gene_length), function(window_i) rep(window_i, gene_length / window_length)))
...
ggplot(div, aes(window, div, color = gene, group = gene)) +
  geom_vline(xintercept = (gene_length / window_length) / 2, linetype = "dashed", color = "red") +
  geom_line(alpha = 0.1) +
  facet_wrap(~ pop)
```
