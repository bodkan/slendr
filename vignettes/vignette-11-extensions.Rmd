...
slim(model, sequence_length = 1e6, recombination_rate = 1e-8, path = output_dir)
```

```{r, eval = FALSE}
directory <- tempdir()
slim(model, sequence_length = 1e6, recombination_rate = 0, path = directory, ts = FALSE, random_seed = 42)
```
...
run_model <- function(origin_pop, onset_time) {
  ...
  slim(model, sequence_length = 1e6, recombination_rate = 0,
       path = directory, ts = FALSE, random_seed = 42)
}
...
load_traj <- function(origin_pop) {
  df <- read.table(file.path(directory, paste0("traj_EUR_", origin_pop, ".tsv")), header = TRUE)
...
initialize() {
    ...
    defineConstant("output_file", PATH + "/" +
...
```{r, eval = RERUN | !file.exists("~/Projects/introgression_data/frequencies.tsv")}
x = Sys.time()
...
```{r, eval = file.exists("~/Projects/introgression_data/frequencies.tsv")}
freqs <- read_tsv("~/Projects/introgression_data/frequencies.tsv")
```

```{r introgression_freqs, eval = file.exists("~/Projects/introgression_data/frequencies.tsv")}
freqs %>%
mutate(pos = pos %% 5e6) %>%
group_by(pos) %>%
...
```{r, eval = file.exists("~/Projects/introgression_data/slim.trees")}
...
```{r introgression_divergence, eval = file.exists("~/Projects/introgression_data/slim.trees")}
ggplot(div_df) +
