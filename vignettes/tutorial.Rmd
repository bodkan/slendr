---
title: "spannr: Introduction and tutorial"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteEncoding{UTF-8}
  %\VignetteIndexEntry{spannr: Introduction and tutorial}
...
migration, and expansion. The package `spannr` is designed to do
...
it. _spannr_ is not yet on CRAN (but will be soon!), so you will need
...
Some of the R libraries that _spannr_ uses internally rely on non-R
...
issue on [Github](https://github.com/bodkan/spannr/issues). Thank you!
...
infrastructure could be used. For now the `spannr` package implicitly
...
This is the current approach of `spannr`: let the user specify
...
additional components. This is unlike other `spannr` object described
...
The `spannr` package implements its own [generic
...
purpose of internal `spannr` machinery:
...
a hidden attribute to each `spannr` object and represents the context
...
such object must carry the same world context - `spannr` complains
...
One of the aims of the _spannr_ package is formalizing the
...
the polygons. All distance units in the _spannr_ package (such as the
...
see how _spannr_ implements admixture of overlapping (but also
...
The way `spannr` implements admixture events is by calling the
...
overlap in population ranges. If we try to instruct `spannr` to
...
case, which can be done by `spannr`'s `plot()` function and the
...
The most crucial function of `spannr` is `compile()`. It takes all
...
by the backend SLiM script provided by `spannr` (more on that below).
...
whole purpose of `spannr` is to let you work on much higher level of
...
In case you'd want to separate model specification and running into different scripts, _spannr_ includes a function `read()` just for this purpose:
...
other demographic changes. While _spannr_ tries to make the formal
...
_spannr_ graph, a single population can participate in many admixture
...
"spannr")`, in case you'd like to peek into its internals).
...
In case we instructed `spannr` to track ancestry proportions, we can
...
Similarly, we can recapitulate the spatial dynamics with the _spannr_
