...
ana <- population(
  ...
  center = c(34, 38), radius = 700e3, polygon = anatolia
) %>% expand(
  by = 2500e3,
  start = 10000,
  end = 7000,
...
model <- compile(
  ...
  competition_dist = 200e3, mate_dist = 200e3, offspring_dist = 50e3,
  overwrite = TRUE
)
```
...
```{r}
read.table("/tmp/test-model/populations.tsv", header = T)
```
