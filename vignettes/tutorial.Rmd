---
title: "stamp: Introduction and tutorial"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteEncoding{UTF-8}
  %\VignetteIndexEntry{stamp: Introduction and tutorial}
...
migration, and expansion. The package `stamp` is designed to do
...
it. _stamp_ is not yet on CRAN (but will be soon!), so you will need
...
Some of the R libraries that _stamp_ uses internally rely on non-R
...
```{r, eval = F}
devtools::install_github("bodkan/stamp")
```
...
issue on [Github](https://github.com/bodkan/stamp/issues). Thank you!
...
infrastructure could be used. For now the `stamp` package implicitly
...
This is the current approach of `stamp`: let the user specify
...
additional components. This is unlike other `stamp` object described
...
The `stamp` package implements its own [generic
...
purpose of internal `stamp` machinery:
...
a hidden attribute to each `stamp` object and represents the context
...
such object must carry the same world context - `stamp` complains
...
One of the aims of the _stamp_ package is formalizing the
...
the polygons. All distance units in the _stamp_ package (such as the
...
see how _stamp_ implements admixture of overlapping (but also
...
the spatial range of Europeans ("Nolithic" population at the time):
...
## Plotting multiple `stamp` objects
...
```{r plot_maps, fig.width = 7, fig.height = 9}
...
The way `stamp` implements admixture events is by calling the
...
overlap in population ranges. If we try to instruct `stamp` to
...
parameters or add `overlap = F` which will instruct stamp to simulate
admixture without spatial overlap.
```

The error message instructs us to visually verify that this is the
case, which can be done by `stamp`'s `plot()` function and the
...
The most crucial function of `stamp` is `compile()`. It takes all
...
by the backend SLiM script provided by `stamp` (more on that below).
...
whole purpose of `stamp` is to let you work on much higher level of
...
In case you'd want to separate model specification and running into different scripts, _stamp_ includes a function `read()` just for this purpose:
...
other demographic changes. While _stamp_ tries to make the formal
...
_stamp_ graph, a single population can participate in many admixture
...
"stamp")`, in case you'd like to peek into its internals).
...
run(
  ...
  how = "batch" # run the model in batch mode (default is SLiMgui)
)
```

## Post-simulation diagnostics

In case we instructed `stamp` to track ancestry proportions, we can
...
Similarly, we can recapitulate the spatial dynamics with the _stamp_
