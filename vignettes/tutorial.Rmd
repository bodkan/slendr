...
<font color = "red"><b>This vignette has not yet been updated to the latest `spammr` version</b></font>
...
## Updating population manually

In case we need a more fine-grained control over a population boundary, we can use the function `update()`. Calling this function on a previously defined `spammr_pop` object, we can update the effective population size of the population at that point such as in this example:

```{r}
whg %>% update(time = 20000, Ne = 3000)
```

Alternatively, we can also specify a new spatial population boundary, using the same arguments used for defining population boundaries in general. Of course, the `Ne` parameter does not have to be specified.

```{r}
whg %>%
  update(time = 7000, Ne = 3000,
         center = c(20, 40), radius = 500)
```
...
greyscale bitmaps. But not only that: we also need to save all the information about population splits, effective population changes, and admixture events. The key here is the function `compile()`. Its sole purpose is to compile the entire model specification and save it in a format which can be loaded by a SLiM backend script provided by the `spammr` package. To compile the model specified in this vignette, we would run:

```{r}
compile(
  ooa, whg, ana, yam, neol,
  output_dir = "model/",
  overwrite = TRUE
)
```

This function converts all vector spatial maps defined above into a rasterized bitmap form and saves all additional information as tables into a directory "model/". If we look into this directory, we will see the following files:

```{r}
list.files("model/")
```

All the numbered PNG files represent rasterized spatial maps, numbered based on their occurence during the simultion, and the TSV files in the directory represent information about population splits, admixture events, and times at which spatial maps should be updated. In practice, you don't have to worry about these details, but it might be instructive to take a look at the generated files and see the immediate connection to the models we defined in the vignette above.

The bottom line is that all these files is what will be ultimately loaded into SLiM, using our back-endSLiM script which will swap different spatial maps in and out, as needed throughout the course of the simulation. We can automatically feed this information into this SLiM script by running the following, which will also pop up the SLiMgui program ready to start the simulation:
