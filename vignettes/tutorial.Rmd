...
In case you'd want to separate model specification and running into different scripts, _spammr_ includes a function `load()` just for this purpose:

```{r}
loaded_model <- load("/tmp/test-model")
```
...
run(
  ...
  max_distance = 500, max_spread = 50,
