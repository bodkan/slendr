...
see how _spannr_ implements geneflow of overlapping (but also
...
The way `spannr` implements geneflow events is by calling the
...
`plot(eur, afr, pop_facets = F)` and either adjust the geneflow
parameters or add `overlap = F` which will instruct spannr to simulate
geneflow without spatial overlap.
```
...
geneflows <- list(
  ...
  geneflow(from = yam_migr, to = eur, rate = 0.75, start = 4000, end = 3000)
)
```
...
```{r}
read.table("/tmp/test-model/geneflows.tsv", header = T)
```
...
_spannr_ graph, a single population can participate in many geneflow
