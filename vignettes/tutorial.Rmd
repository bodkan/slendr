...
The 'rendered' form is what is ultimately exported in a serialized form
...
To visually see what is really going on behind the scene, we can produce
the complete, unrendered form of the expansion:
...
```{r}
population(
  "nomadic population", time = 10000, world,
  ...
  plot(rendering = F, title = "Migration along a trajectory")
```
...
spatio-temporal population models for the purpose of forward simulations
in SLiM. The crucial piece of machinery which connects the `spammr` R
package with SLiM is an Eidos function called `defineSpatialMap()`
which, among other parameters, accepts a two-dimensional matrix
(essentially, a raster image) describing a spatial simulation landscape.
See SLiM manual for more information.
...
greyscale bitmaps The key is a `spammr` function `rasterize()`.
...
```{r, eval = F}
list.files("/tmp/model")
```

```{r, include=FALSE}
unlink("/tmp/model", recursive = TRUE, force = FALSE)
```
