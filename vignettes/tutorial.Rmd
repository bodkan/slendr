...
Some of the R libraries that _spammr_ uses internally rely on non-R software that needs to be installed before doing anything else. At the very least, you should make sure to install `udunits` and `gdal` on your system (if you're on a Mac, I recommend [homebrew](https://brew.sh/)). In my experience, some early users had those already installed without even knowing it (perhaps because they already used tools or R packages depend)
...
`admixture()` function. This function has a very straightforward interface which you should be able to understand from the examples below.
...
The error message instructs us to visually verify
that this is the case, which can be done by `spammr`'s `plot()` function and the
