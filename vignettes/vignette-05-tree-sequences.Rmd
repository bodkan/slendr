...
anc <- population("ANC", time = 1e6, N = 1)
nea <- population("NEA", parent = anc, time = 500e3, N = 1000, remove = 40e3)
afr <- population("AFR", parent = anc, time = 500e3, N = 12000)
...
model <- compile(
  ...
  generation_time = 30,
  dir = file.path("/tmp/", "archaic-introgression"), overwrite = TRUE
)
```
...
```{r}
model <- read("/tmp/archaic-introgression/")
ts_file <- file.path(model$path, "output_tree_seq.trees")
file.exists(ts_file)
#file.copy(ts_file, "~/Desktop/ts.trees", overwrite = TRUE)
```
