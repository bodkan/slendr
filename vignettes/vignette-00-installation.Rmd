...
*slendr* is currently in the process of the first CRAN submission which is why it can't be installed with simple `install.packages("slendr")` just yet.

For the moment, **the easiest way to install it is to get its development version directly from GitHub by executing `devtools::install_github("bodkan/slendr")`** via the R package *devtools* ( `install.packages("devtools")`). In fact, if you decide to try *slendr*, please make sure to update it regularly and keep an eye on the [changelog](https://slendr.net/news/index.html) on a regular basis!
...
Note that *slendr* requires SLiM 3.7.1 and will not work with an earlier version. Again, running `library(slendr)` will inform you of any potential issues with your SLiM installation.
...
In case you are wondering how does *slendr* accomplish the above: *slendr*'s Python interface is not a hack that would simply call Python from the command-line in the background, but is implemented using the R package [*reticulate*](https://rstudio.github.io/reticulate/)*.* This embeds a Python session within an R session, enabling high-performance interoperability between both languages.

The R package *reticulate* provides [several alternative ways](https://rstudio.github.io/reticulate/reference/use_python.html) to link R to a custom-created standard Python virtual environment or a custom conda environment. However, I strongly recommend using the R-specific isolated environment created by `setup_env()` to avoid any issues with custom venv/virtualenv/pyenv/pyvenv/pipenv/poetry/conda setups that you might normally prefer for your Python work.

## Docker

There is currently no official Docker image for *slendr* but there will be once the R package finally lands on the official CRAN repository. The current plan is to use the geospatial image published by the [Rocker project](https://www.rocker-project.org/) (which already contains pre-compiled R, RStudio, and all necessary R package dependencies such as *sf* ) and extend it with *slendr* and SLiM.
