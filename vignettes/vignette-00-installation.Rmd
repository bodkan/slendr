...
If you want (or need) to get its development version, you can install it directly from GitHub by executing `devtools::install_github("bodkan/slendr")` via the R package *devtools* (you can gen *devtools* by running `install.packages("devtools")`). In fact, if you decide to try _slendr_, please make sure to update it regularly and keep an eye on the [changelog](https://bodkan.github.io/slendr/news/index.html) on a regular basis! This is where you can find information about latest bugfixes and potential breaking changes.
