...
ts <- ts_load(file.path(model$path, "output_msprime.trees"), model) %>% ts_mutate(mutation_rate = 1e-8, random_seed = SEED)
...
ts_slim <- ts_load(file.path(model$path, "output_slim.trees"), model) %>% ts_mutate(mutation_rate = 1e-8, random_seed = SEED)
