...
```{r, eval = eval_chunk, fig.keep='none'}
...
system(sprintf("cp -r %s %s", model$path, "~/Documents/postdoc/slendr-paper/slendr_models/ex1"))
ts_save(ts, "~/Documents/postdoc/slendr-paper/slendr_models/ex1.trees")
```
...
ggsave("~/Documents/postdoc/slendr-paper/ex1.pdf", p_ex1, width = 9, height = 6, units = "in")
```
...
ts <-
  slim(model, sequence_length = 10e6 / scaling, recombination_rate = 1e-8) %>%
  ts_simplify() %>%
...
```{r, echo = FALSE, eval = eval_chunk}
system(sprintf("cp -r %s %s", model$path, "~/Documents/postdoc/slendr-paper/slendr_models/ex2"))
ts_save(ts, "~/Documents/postdoc/slendr-paper/slendr_models/ex2.trees")
```
...
```{r, include = FALSE, eval = eval_chunk && save_pdf}
ggsave("~/Documents/postdoc/slendr-paper/ex2.pdf", p_ex2, width = 6, height = 9, units = "in")
```
...
```{r, echo = FALSE, eval = eval_chunk}
system(sprintf("cp -r %s %s", model$path, "~/Documents/postdoc/slendr-paper/slendr_models/ex3"))
ts_save(ts, "~/Documents/postdoc/slendr-paper/slendr_models/ex3.trees")
```
...
```{r, include = FALSE, eval = eval_chunk && save_pdf}
ggsave("~/Documents/postdoc/slendr-paper/ex3.pdf", p_ex3, width = 9, height = 6, units = "in")
```

<!-- ```{r} -->
<!-- eurs <- ts_samples(ts) %>% filter(pop == "EUR", time < 3500) %>% pull(name) -->
<!-- eurs <- c("EUR_542", "EUR_549", "EUR_567", "EUR_581", "EUR_582") -->
<!-- eurs <- c("EUR_581") -->

<!-- # finding a EUR individual with illustratively rich spatial ancestry -->
<!-- pdf("~/Documents/postdoc/slendr-paper/anc.pdf") -->
<!-- for (e in eurs) { -->
<!-- ancestors <- ts_ancestors(ts, e) -->
...
<!-- # finding appropriate leaves for plotting an illustrative tree -->
<!-- ggplot() + -->
<!--   geom_sf(data = map) + -->
<!--   geom_sf(data = ancestors, size = 0.5, aes(alpha = parent_time)) + -->
<!--   geom_sf(data = sf::st_set_geometry(ancestors, "parent_location"), -->
<!--           aes(shape = parent_pop, color = parent_pop)) + -->
<!--   geom_sf(data = filter(ts_nodes(ts), name == e), size = 3) + -->
<!--   geom_sf_text(data = filter(ancestors, child_pop == "ANA", !is.na(name)), aes(label = parent_id)) -->
<!--   coord_sf(expand = 0) + -->
<!--   labs(x = "longitude", y = "latitude") + -->
<!--   theme_bw() + -->
<!--   facet_grid(. ~ node_id, labeller = labeller(node_id = chrom_names)) + -->
...
```{r, echo = FALSE, eval = eval_chunk}
ts_save(ts, "~/Documents/postdoc/slendr-paper/slendr_models/ex3_small.trees")
```

### Plotting code

```{r, figure_ex4, eval = eval_chunk, fig.height=9, fig.width=7, class.source = "fold-hide"}
library(ggtree)
...
p_map <- ggplot() +
  ...
  coord_sf(xlim = c(3047066.1, 8688656.9),
...
```{r, include = FALSE, eval = eval_chunk && save_pdf}
ggsave("~/Documents/postdoc/slendr-paper/ex4.pdf", p_ex4, width = 7, height = 9, units = "in")
```
