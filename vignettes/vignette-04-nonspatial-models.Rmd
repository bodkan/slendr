...
```{r}
debugonce(schedule_sampling)

expect_error(schedule_sampling(model, times = 45000, list(afr, 10), list(ooa, 3, "Ust_Ishim")),
             "A named sample must represent only a single individual, i.e. sampling of n = 1")
...
schedule <- schedule_sampling(model, times = 15000, list(eur, 1), list(ehg, 10))
ts_samples(msprime(model, sequence_length = 100000, recombination_rate = 0, samples = schedule))
