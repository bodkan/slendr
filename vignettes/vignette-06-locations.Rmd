...
```{r, include = F}
nodes <- lineages[!is.na(lineages$name), ]$node_id
```
...
2.  The first chromosome (node `r nodes[1]`) only traces its ancestry to Yamnaya indicated by a star. This is expected because we programmed Yamnaya to contribute significant part of ancestry to present-day Europeans (compare to the demographic graph on top of this vignette). In turn, the Yamnaya node traces its ancestry to EHG (which is its parental population). We can see a cluster of EHG nodes in one location because we did not specify any spatial dynamic for this population in our model

3.  The second node of the individual (node `r nodes[2]` in the right panel representing the second chromosome) traces part of its ancestry to the EHG population (filled squares) and part to the Anatolia (triangles). This makes sense, because we have simulated European ancestry as being part Anatolian. In fact, we can see a nice "trace" of nodes (past individuals) of the Anatolian ancestry spreading to Europe over time.
...
distances %>%
  ...
  ggplot(aes(epoch, speed, color = child_pop)) +
  geom_boxplot(color = "black", outlier.shape = NA) +
  geom_jitter(alpha = 0.25) +
  facet_grid(pop ~ child_pop) +
  labs(y = "speed [kilometers / generation]") +
...
distances %>%
  filter(child_time < 60000) %>%
  ggplot(aes(child_time, distance, color = child_pop)) +
  geom_point() +
  facet_grid(pop ~ child_pop) +
  labs(y = "distance [kilometers]") +
  theme(axis.text.x = element_text(hjust = 1, angle = 45))
```
...
distances %>%
  filter(child_time < 60000) %>%
  ggplot(aes(child_time, speed, color = child_pop)) +
  geom_point() +
  facet_grid(pop ~ child_pop) +
  labs(y = "speed [kilometers per generation]") +
  theme(axis.text.x = element_text(hjust = 1, angle = 45))
```
...
distances %>%
  ...
  ggplot(aes(child_time, distance, color = child_pop)) +
  geom_point() +
  geom_smooth(aes(group = child_pop)) +
  labs(y = "distance [kilometers]") +
  theme(axis.text.x = element_text(hjust = 1, angle = 45))
```
...
directions <- lineages %>%
  ...
  mutate(epoch = cut(child_time, breaks = c(0, 3000, 4000, 5000, 20000, 30000, 40000, 50000, Inf), include.lowest = TRUE, dig.lab = 10)) %>%
#  mutate(epoch = cut(child_time, breaks = c(Inf, seq(60000, 0, by = -5000)), include.lowest = TRUE, dig.lab = 10)) %>% 
  ...
  select(name, pop, node_id, horizontal, vertical,
