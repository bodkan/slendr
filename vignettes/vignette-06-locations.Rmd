...
```{r}
slim(
  model, seq_length = 100e3, recomb_rate = 1e-8, burnin = 150000,
...
ts_draw(ts$first())
ts_draw(ts$first(), individuals = TRUE, ts = ts)

ts2 <- ts_simplify(ts, c("AFR1", "OOA1", "EUR50", "EHG12"))

ts_draw(ts2$first())
ts_draw(ts2$first(), individuals = TRUE, ts = ts2)

ts_sf <- ts_spatial(ts)

plot_parents(ts_sf, "EUR1")
