...
afr <- population(
  ...
  competition_dist = 250e3, mate_dist = 200e3, dispersal_dist = 70e3,
...
# European population
eur <- population(
  name = "EUR", parent = ehg, time = 25000, N = 5000, polygon = europe, competition_dist = 400e3, mate_dist = 200e3, dispersal_dist = 25e3
)
...
model <- compile(
  ...
  competition_dist = 200e3, mate_dist = 200e3, dispersal_dist = 70e3,
  dir = file.path(tempdir(), "spatial-ts"), overwrite = TRUE
)
```
...
```{r, model_graph, fig.width = 7, fig.height = 7}
plot_graph(model)
```
...
Now we will schedule the sampling of a single individual from each population every two thousand years, starting from 40 thousand years ago all the way to the present (this is a feature discussed in the basic [tree sequence overview](vignette-05-tree%20sequences.html#scheduling-of-sampling-events-1)):
...
Because we record the time and the location of every individual that happens to be the ancestor of at least one sampled individual, this means that, looking at the following tree, for instance, we know the true location of every node in this tree (and all other genealogies in the tree sequence as well):

```{r, ts_small_tree, echo = FALSE}
ts_small <- ts_simplify(ts, c("AFR1", "EUR1", "YAM1", "EHG1", "OOA1", "ANA1"))

ts_tree(ts_small, 1) %>% ts_draw(labels = TRUE, ts = ts_small)
```
...
```{r, include = FALSE}
ana_ind <- ts_samples(ts) %>% filter(name == "ANA22")
```
...
Now we can plot the absolute distances "travelled" by nodes from the locations of their parents:

```{r distr_abs_distances, fig.width = 9, fig.height = 6}
distances_long %>%
  filter(stat == "distance") %>%
  ggplot(aes(epoch, value, color = child_pop)) +
  geom_jitter(alpha = 0.25, size = 0.5) +
  geom_boxplot(color = "black", outlier.shape = NA) +
  facet_grid(pop ~ child_pop) +
  labs(y = "absolute distance of node from parent [km]",
       x = "time of a node [years ago]") +
  theme(axis.text.x = element_text(hjust = 1, angle = 45, size = 6),
        legend.position = "bottom") +
  guides(color = guide_legend("ancestral node\npopulation"))
```

Similarly, we can take a look at how fast those nodes must have travelled (i.e. the distance they travelled per generation):


```{r distr_speed, fig.width = 9, fig.height = 6}
distances_long %>%
  filter(stat == "speed") %>%
  ggplot(aes(epoch, value, color = child_pop)) +
  geom_jitter(alpha = 0.25, size = 0.5) +
  geom_boxplot(color = "black", outlier.shape = NA) +
  facet_grid(pop ~ child_pop) +
  labs(y = "distance traveled per generation [km]",
       x = "time of a node [years ago]") +
  theme(axis.text.x = element_text(hjust = 1, angle = 45, size = 6),
        legend.position = "bottom") +
  guides(color = guide_legend("ancestral node\npopulation")) +
...
### Smoothed fits of travelled distances

Still keeping an eye on the animated data above, let's try to summarise the information about distances "traveled" by nodes in different time period by fitting a spline (rather than plotting the raw data with individual nodes):
...
- TODO: Heatmaps of ancestry locations (similar to the discrete node location maps)
- TODO: Effective population size heatmaps?

```{r, eval = FALSE, echo = FALSE}
nodes_sf <-
  as.data.frame(lineages) %>%
  distinct(parent_id, .keep_all = TRUE) %>%
  sf::st_as_sf() %>%
  sf::st_set_geometry("parent_location")

ggplot() +
  geom_sf(data = map) +
  geom_sf(data = nodes_sf, aes(color = parent_time), alpha = 0.5)
```
