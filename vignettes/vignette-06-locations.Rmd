...
```{r}
slim(
  model, seq_length = 100e3, recomb_rate = 1e-8,
...
```{r}
ts2 <- ts_load(model, simplify = TRUE)
ts2
```
...
ts2 <- ts_recapitate(ts1, recombination_rate = 1e-8, Ne = 10000)
ts_draw(ts2$first())

ts3 <- ts_simplify(ts2)
ts_draw(ts3$first())

ts4 <- ts_simplify(ts2, individuals = c("AFR1", "OOA1", "EHG13", "ANA1",
                                        "YAM4", "EUR5"))
ts_draw(ts4$first())
ts_draw(ts4$first(), individuals = TRUE, ts = ts4)

ts = ts3

ts_sf <- ts_spatial(ts)

plot_ancestors(ts_sf, "EUR50")
