...
nea <- population("NEA", parent = anc_arch, time = 400e3, N = 100, remove = 30e3)
den <- population("DEN", parent = anc_arch, time = 400e3, N = 100, remove = 30e3)
nonafr <- population("ancestor_nonAFR", parent = afr, time = 100e3, N = 1000, remove = 39e3)
...
plot_model(
  model, sizes = FALSE,
  order = c("AFR", "EUR", "ancestor_nonAFR", "PAP", "ancestor_all", "DEN", "ancestor_archaics", "NEA")
)
```
...
ts <- msprime(model, sequence_length = 100e6, recombination_rate = 1e-8, samples = samples, random_seed = 123)
```
