...
```{r, eval = !file.exists("~/Desktop/slendr/ex1/output_msprime.trees")}
SEED <- 42
set.seed(SEED)
...
# result is visualized in panel B
...
```{r, include = FALSE, eval = !file.exists("~/Desktop/slendr/ex1/output_msprime.trees"), class.source = "fold-hide"}
...
```{r, include = FALSE, eval = file.exists("~/Desktop/slendr/ex1/output_msprime.trees"), class.source = "fold-hide"}
model <- read_model("~/Desktop/slendr/ex1/")
ts <- ts_load(model, file = file.path(model$path, "output_msprime.trees")) %>% ts_mutate(mutation_rate = 1e-8)
...
```{r, figure_ex1, fig.height=6, fig.width=9, class.source = "fold-hide"}
...
ggsave("~/Documents/postdoc/slendr_paper/ex1.pdf", p_ex1, width = 9, height = 6, units = "in")
```
...
```{r, eval = !file.exists("~/Desktop/slendr/ex2/output_slim.trees")}
SEED <- 42
set.seed(SEED)
...
model <- compile_model(
  ...
  path = "~/Desktop/slendr/ex2", overwrite = T, force = T
)
...
```{r, include = FALSE, eval = file.exists("~/Desktop/slendr/ex2/output_slim.trees")}
model <- read_model("~/Desktop/slendr/ex2")
map <- model$world
...
```{r, figure_ex2, fig.height=9, fig.width=6, class.source = "fold-hide"}
...
ggsave("~/Documents/postdoc/slendr_paper/ex2.pdf", p_ex2, width = 6, height = 9, units = "in")
```
...
```{r, eval = !file.exists("~/Desktop/slendr/ex3/output_slim.trees")}
map <- world(xrange = c(-15, 60), yrange = c(20, 65), crs = 3035)
...
model <- compile_model(
  path = "~/Desktop/slendr/ex3", overwrite = TRUE, force = TRUE,
...
```{r, include = FALSE, eval = file.exists("~/Desktop/slendr/ex3/output_slim.trees")}
model <- read_model("~/Desktop/slendr/ex3")
map <- model$world
```

```{r, figure_ex3, fig.height=5, fig.width=7, class.source = "fold-hide"}
...
ggsave("~/Documents/postdoc/slendr_paper/ex3.pdf", p_ex3, width = 9, height = 6, units = "in")
```
...
```{r, figure_ex4, fig.height=9, fig.width=7, class.source = "fold-hide"}
chrom_names <- setNames(
  c("EUR_599 (chromosome 1)", "EUR_599 (chromosome 2)"),
...
p_tree <- ggtree(tree, aes(color = pop, fill = pop)) %<+% df +
  geom_tiplab(align = TRUE, geom = "label",
...
ggsave("~/Documents/postdoc/slendr_paper/ex4.pdf", p_ex4, width = 7, height = 9, units = "in")
```
