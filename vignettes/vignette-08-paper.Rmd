...
# f4ratio_slim %>% inner_join(samples, by = c("X" = "name")) %>% ggplot(aes(pop, alpha, color = pop)) + geom_boxplot() + geom_jitter() + geom_hline(yintercept = 0) + coord_cartesian(ylim = c(0, 0.2))
```

```{r, include = FALSE, eval = file.exists("~/Desktop/slendr/ex1/output_msprime.trees")}
SEED <- 42
set.seed(SEED)
...
```{r, figure_ex1, fig.height=3, fig.width=6, class.source = "fold-hide"}
...
p_ex3_f4ratio <- f4ratio_both %>%
  ...
  #                  alpha = 0.5, size = 0.25, outlier.shape = NA) +
