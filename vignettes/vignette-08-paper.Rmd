...
ooa <- population(
  "OOA", time = 50000, N = 2000, remove = 23000,
   map = map, center = c(33, 30), radius = 400e3
) %>%
  move(trajectory = ooa_trajectory, start = 50000, end = 40000)

ehg <- population(
  "EHG", time = 28000, N = 3000, parent = ooa, remove = 6000,
  map = map, polygon = R1
)

eur <- population(
  "EUR", time = 30000, N = 3000, parent = ooa,
  map = map, polygon = R2
) %>%
  resize(N = 10000, time = 5000, end = 0, how = "exponential")

ana <- population(
  "ANA", time = 22000, N = 3000, parent = ehg, remove = 3000,
  map = map, polygon = R3
) %>%
  expand_range(by = 3e6, start = 10000, end = 7000, polygon = R4)

yam <- population(
  "YAM", time = 7000, N = 3000, parent = ehg, remove = 2500,
  map = m, polygon = R5
) %>%
...
samples <- schedule_sampling(
  ...
  list(ehg, 1), list(ana, 1), list(yam, 1), list(eur, 20)
)
...
plot_grid(
  plot_map(model) + guides(alpha = "none"),
  plot_model(model),
  labels = c("B", "C"), nrow = 2
)
```
