...
model <- compile_model(
  ...
  generation_time = 1, sim_length = 6000, path = "~/Desktop/slendr/ex1", overwrite = TRUE, force = TRUE
)
...
# f4ratio %>% inner_join(samples, by = c("X" = "name")) %>% ggplot(aes(pop, alpha, color = pop)) + geom_boxplot() + geom_jitter() + geom_hline(yintercept = 0) + coord_cartesian(ylim = c(0, 0.2))
```
...
ts_small <- ts %>% ts_simplify(
  c("EUR_599", "ANA_322", "EHG_7", 
...
p_tree <- ggtree(tree, aes(color = pop, fill = pop)) %<+% df +
  geom_tiplab(align = TRUE, linetype = "dotted", hjust = -0.1) +
  geom_label2(aes(label = label, subset = !isTip), color = "black") +
  ...
  theme(legend.position = "bottom") +
  scale_x_continuous(limits = c(-55000, 10000), labels = abs); revts(p_tree)
