...
1.  *Spatial competition distance* - governed by *slendr* parameter `competition,` which signifies the maximum distance at which two individuals can influence each other's fitness via competition.
...
```{r plot_gif_interactions, message = FALSE, eval = Sys.which("slim") != "" && env_present && Sys.getenv("RUNNER_OS") != "macOS" && Sys.getenv("R_DEVEL_LOCAL") == TRUE && Sys.getenv("NOT_CRAN") != "true"}
...
```{r plot_interactions, eval = Sys.which("slim") != "" && env_present && Sys.getenv("RUNNER_OS") != "macOS" && Sys.getenv("R_DEVEL_LOCAL") == TRUE && Sys.getenv("NOT_CRAN") == "true"}
library(ggplot2)

locations <- ts_load(model) %>% ts_nodes() %>% dplyr::filter()
ggplot() + geom_sf(data = locations, aes(color = pop)) + theme_minimal()
```
