...
In addition to building models that capture movement or expansion using a set of [simple R functions](https://bodkan.github.io/slendr/reference/index.html#section-population-dynamics), _slendr_ makes it possible to program more subtle parameters of spatial population dynamics.
...
```{r plot_interactions, eval = slendr::check_dependencies(python = TRUE, slim = TRUE)}
library(ggplot2)
