...
#' @param geneflows Geneflow events generated by the \code{geneflow} function
...
compile <- function(populations, dir, generation_time, resolution,
                    ...
                    geneflows = NULL, overwrite = FALSE) {
  ...
  result <- list(
    config = list(
      ...
      geneflows = if (is.null(geneflows)) NULL else file.path(dir, "geneflows.tsv")
    ),
    world = map,
...
read <- function(dir) {
  ...
  path_geneflows <- file.path(dir, "geneflows.tsv")
...
and generation_time.txt are all present", dir), call. = FALSE)
  ...
  # was specified)
  admix_table <- NULL
  if (file.exists(path_geneflows)) {
    admix_table <- read.table(path_geneflows, header = TRUE, stringsAsFactors = FALSE)
  ...
  result <- list(
    config = list(
      ...
      splits = path_splits,
      maps = path_maps,
      geneflows = if (is.null(admix_table)) NULL else file.path(dir, "geneflows.tsv")
    ),
