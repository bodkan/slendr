...
compile_maps <- function(populations, splits_table, pixel) {
  # generate rasterized maps
  maps <- render(populations, pixel)
...
#' @param pixel Two-dimensional numeric vector specifying the
#'   absolute dimension of the raster map
#' @param overwrite Overwrite the contents of the output directory (in case it
#'   already exists)?
#'
#' @export
compile <- function(populations, output_dir, admixtures = NULL, pixel, overwrite = FALSE) {
  ...
  maps_table <- compile_maps(populations, splits_table, pixel)
...
render <- function(pops, pixel) {
  raster_list <- lapply(pops, function(pop) {
    ...
    snapshots <- lapply(unique(pop$time), function(t) {
      ...
      raster_map <- rasterize(snapshot, pixel)
...
rasterize <- function(x, pixel) {
  ...
  template <- stars::st_as_stars(bbox, dx = pixel, dy = pixel)
