...
admixture <- function(from, to, rate, start, end, minimum = 0.2) {
  ...
  # make sure the population is not removed during the admixture period
  from_remove <- attr(from, "remove")
  to_remove <- attr(to, "remove")
  if (from_remove > start | from_remove > end) {
    stop(sprintf(
      "Population %s scheduled for removal at time %d, which is
outside of the %d-%d admixture time range",
...
`plot(%s, %s, pop_facets = F)` or explicitly force non-overlapping
admixture by adding `minimum = 0` to the function call.",
