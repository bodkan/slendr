...
#'
#' First, compiles the vectorized population spatial maps into a series of binary raster PNG files,
#' which is the format that SLiM understands and uses it to define population boundaries. Then
#' extracts the demographic model defined by the user (i.e. population divergences and gene flow
#' events) into a series of tables which are later used by the built-in SLiM script to program the
#' timing of simulation events.
#'
#' @param populations Object(s) of the \code{slendr_pop} class (multiple objects need to be
...
#' @param geneflow Geneflow events generated by the \code{geneflow} function (either a list of
#'   data.frame objects in the format defined by the \code{geneflow} function, or a single
#'   data.frame)
#' @param competition_dist,mate_dist Maximum spatial competition and mating choice distance
#' @param offspring_dist Standard deviation of the normal distribution of the parent-offspring
#'   distance
#' @param dir Output directory for the model configuration files which will be loaded by the backend
#'   SLiM script
#' @param overwrite Overwrite the contents of the output directory in case it already exists?
#' @param sim_length Total length of the simulation (if time is specified in a forward direction)
#'
