...
#'
#' @param populations Population map objects of the \code{stamp_pop} class (a
...
#' Read a previously serialized stamp model
#'
...
read <- function(model_dir) {
  ...
  if (!all(file.exists(c(path_populations, path_splits, path_maps, path_names))))
    stop(sprintf("Directory '%s' does not contain all stamp configuration files.
...
run <- function(model, sim_length, seq_length, recomb_rate,
  ...
  if (!all(file.exists(file.path(model_dir, c("splits.tsv", "maps.tsv")))))
    stop(sprintf("Directory '%s' does not contain stamp configuration files", model_dir), call. = FALSE)

  if (!length(list.files(model_dir, pattern = "*.png") == 0))
    stop(sprintf("Directory '%s' does not contain any stamp spatial raster maps", model_dir), call. = FALSE)
...
a non-zero integer number (number of neutral ancestry markers)", call. = FALSE)
  ...
  backend_script <- system.file("extdata", "backend.slim", package = "stamp")
...
#'
#' @param pops Spatial population objects of the 'stamp_pop' class
#'
#' @import ggplot2
...
#'
#' @param x Object of the 'stamp_pop' class
#' @param resolution How many kilometers per pixel?
