...
setup_env <- function(quiet = FALSE, agree = FALSE, pip = FALSE) {
  if (is_slendr_env_present()) {
    ...
    if (answer == 2) {
      ...
      which_tspop_pyslim_pandas <- grepl("tspop|pyslim|pandas", package_versions)
      reticulate::conda_install(envname = PYTHON_ENV, packages = package_versions[!which_tspop_pyslim_pandas], pip = pip)
      reticulate::conda_install(envname = PYTHON_ENV, packages = c(package_versions[which_tspop_pyslim_pandas], "pyarrow"), pip = TRUE)
