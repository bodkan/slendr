...
read_ancestries <- function(output_prefix) {
  output_prefix <- paste0(output_prefix, "_ancestry")
  output_dir <- dirname(output_prefix)
  file_prefix <- basename(output_prefix)
  files <- list.files(output_dir, pattern = file_prefix, full.names = TRUE)
  if (!length(files))
    stop("No ancestry proportion tables found. Did you specify the correct path?",
         call. = FALSE)

  purrr::map_dfr(files, function(f) {
    anc_df <- readr::read_tsv(
      f, progress = FALSE, show_col_types = FALSE,
      col_types = readr::cols(.default = readr::col_double())
    ) %>%
      dplyr::mutate(gen = as.integer(gen))
