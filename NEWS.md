...
- For reading tree sequences from a file (optionally while annotating them with information from a _slendr_ model which produced them), users should now use a function `ts_read()`. Similarly, the function `ts_write()` now serves to write a tree-sequence to a file. This applies both to tree-sequences produced by _slendr_ models as wel as tree-sequences generated by other means (non-_slendr_ simulations, manually created tree sequences, etc.). ([#e9d3bd](https://github.com/bodkan/slendr/commit/e9d3bd))
